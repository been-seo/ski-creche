"""Ski Creche on FineWeb-Edu: Snowball vs E2E baseline."""
import math
...
DB_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'fineweb.db')
...
class Embed(nn.Module):
    def __init__(self, vocab_size, d, seq_len):
        ...
        # Learnable pos embedding is dead in forward_single because
        # h.detach() severs the gradient path before the first block.
...
class Readout(nn.Module):
    def __init__(self, d, embed_weight):
        ...
        # bypass nn.Module registration to avoid double-counting params
