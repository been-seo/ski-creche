...
the main run): $N \approx 3.2\times 10^{6}$ (the embedding alone is
$2.3{\times}10^6$ parameters), $\sigma_{\text{eval}} \approx 0.05$,
giving $\eta_{\max} \approx 2.8\times 10^{-5}$ from
Eq.~\ref{eq:lr-bound} but $\eta_{\max} \approx 6{\times}10^{-4}$
after the empirical $\|g\|_1/\|g\|_2$ correction --- consistent with
the sweet spot $\mathrm{LR}/2 = 1.5\times 10^{-4}$ that the
diagnostic sweep at fixed $d{=}46$
(Appendix~\ref{app:lr-sweep}) identifies as best (val $4.84$ at the
end of the sweep window; $\mathrm{LR}=3\times 10^{-4}$ drifts
upward, $\mathrm{LR}/3$ shows occasional gradient spikes,
...
$n_{\text{heads}}^{\text{new}} = d_{\text{new}}/d_{\text{head}}^{\text{new}}$
is integer.  Weight matrices $W_q, W_k, W_v, W_o$ are
\emph{stored} as $(d, d)$ Linears; the head decomposition is purely
a \texttt{view} for the per-head reshape inside the attention
forward pass.  Doubling $d_{\text{head}}$ therefore changes only the
\emph{view} stride, not the stored weights: a column previously
interpreted as the last entry of head $k$ becomes the first entry
of head $\lfloor k/2 \rfloor$'s second half.  This re-partitioning
is not function-preserving (the softmax-over-head-index splits
differ), but the new $d$-columns produced by the grow are
zero-padded with the asymmetric init of
\S\ref{sec:asymmetric-init}, so the perturbation is the same kind
of one-time shock as a regular grow event.  We treat the
$d_{\text{head}}$ transition step as one extra grow event:
$\texttt{in\_recovery}$ is set for $W$ steps and the divergence
guard is \emph{not} reset (best-so-far is preserved across
transitions, since the function shock typically pushes val above
best).  Up to $d{=}2048$ there are at most four such transitions
